...
\begin{definition}[Tree represented by an attack rule]
...
\begin{definition}[Partition of an attack tree]
  ...
  \begin{enumerate} 
    ...
    %\item $R$ is a subtree of $T$. % define subtree
    %\item With $S = \{ root(R') \mid R' ∈ P ∧ R'\text{ is descendant of }R ∧ R' ≠ R\}$
    %  being the descendants of $R$, not including itself, then $open(R) = S$
  \end{enumerate}
\end{definition}
